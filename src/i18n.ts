...
const DATA: Record<Lang, LangData> = {
  cs: {
    msgs: {
      ...
      advHint: 'Vyplň políčka: řádek = den, sloupec = týden cyklu (1. týden = týden, kdy cvičení vzniklo). Každý den může mít jiný čas a každý týden jiné dny. Prázdné políčko = ten den se v daném týdnu netrénuje.',
  ...
  en: {
    msgs: {
      ...
      advHint: 'Fill the cells: row = weekday, column = week of the cycle (week 1 = the week the exercise was created). Each day can have its own time and each week its own days. An empty cell means no workout that day in that week.',
  ...
  de: {
    msgs: {
      ...
      advHint: 'Fülle die Zellen aus: Zeile = Wochentag, Spalte = Woche des Zyklus (Woche 1 = die Woche, in der die Übung erstellt wurde). Jeder Tag kann eine eigene Zeit haben, jede Woche eigene Tage. Leere Zelle = an diesem Tag in dieser Woche kein Training.',
