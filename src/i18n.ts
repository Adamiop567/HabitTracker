...
const DATA: Record<Lang, LangData> = {
  cs: {
    msgs: {
      ...
      advHint: 'Vyplň políčka: řádek = den, sloupec = týden cyklu (1. týden = týden, kdy cvičení vzniklo). Každá buňka = jeden trénink se začátkem a koncem; každý den může mít jiný čas a každý týden jiné dny. Prázdný začátek = ten den se v daném týdnu netrénuje.',
      advFillAll: 'Vyplnit vše časem',
      advClearAll: 'Vyprázdnit vše',
      advWeekNow: 'Dnešní týden odpovídá sloupci {n}.',
      advCellHint: 'V každé buňce: horní pole = začátek, dolní (se zvýrazněným okrajem) = konec. Prázdný konec použije konec z pole „Konec“ pod mřížkou, jinak trénink konce nemá.',
  ...
  en: {
    msgs: {
      ...
      advHint: 'Fill the cells: row = weekday, column = week of the cycle (week 1 = the week the exercise was created). Each cell is one workout with a start and an end; each day can have its own time and each week its own days. An empty start means no workout that day in that week.',
      advFillAll: 'Fill all with time',
      advClearAll: 'Clear all',
      advWeekNow: 'This week corresponds to column {n}.',
      advCellHint: 'In each cell: top field = start, bottom one (highlighted edge) = end. An empty end uses the “End” field below the grid; otherwise the workout has no end.',
  ...
  de: {
    msgs: {
      ...
      advHint: 'Fülle die Zellen aus: Zeile = Wochentag, Spalte = Woche des Zyklus (Woche 1 = die Woche, in der die Übung erstellt wurde). Jede Zelle ist ein Training mit Beginn und Ende; jeder Tag kann eine eigene Zeit haben, jede Woche eigene Tage. Leerer Beginn = an diesem Tag in dieser Woche kein Training.',
      advFillAll: 'Alle mit Zeit füllen',
      advClearAll: 'Alles leeren',
      advWeekNow: 'Diese Woche entspricht Spalte {n}.',
      advCellHint: 'In jeder Zelle: oberes Feld = Beginn, unteres (mit hervorgehobenem Rand) = Ende. Leeres Ende verwendet das Feld „Ende“ unter dem Raster, sonst hat das Training kein Ende.',
